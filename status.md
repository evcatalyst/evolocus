...
- Deployed visual smoke workflow: manual GitHub Actions route smoke can click the hosted Pages URL through Chart -> Map -> Inquiry -> Ontology, ontology tier share URL replay, latest-analysis Ask-this-map-layer, question-to-map highlight depth, and selected-unit ontology route comparison without secrets or write permissions
- Visual smoke public artifact: `visual_smoke.json` records workflow metadata and no-row/no-text/no-secret policy flags for verified public progressive routes, including ontology tier share URLs, latest-analysis Ask-this-map-layer, question-to-map highlight depth, and selected-unit ontology route comparison
