...
- Visual route verification: GitHub Pages displays `visual_smoke.json` as a first-screen card linking to the successful hosted browser-smoke run for Chart -> Map -> Inquiry -> Ontology, ontology tier share URLs, and latest-analysis Ask-this-map-layer
...
- Deployed visual smoke workflow: manual GitHub Actions route smoke can click the hosted Pages URL through Chart -> Map -> Inquiry -> Ontology, ontology tier share URL replay, and latest-analysis Ask-this-map-layer without secrets or write permissions
- Visual smoke public artifact: `visual_smoke.json` records workflow metadata and no-row/no-text/no-secret policy flags for verified public progressive routes, including ontology tier share URLs and latest-analysis Ask-this-map-layer
